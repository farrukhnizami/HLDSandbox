...
Trigger: Manager Approve action on the FDS Over time timesheet screen

. Delivery Office Manager (COM) opens the Timesheet Screen
. Frontend → Backend → Timesheet Service API → FDS DB (fetch unapproved shifts)
...
. Status flows back up: FDSDB → API → Backend → Frontend (manager sees updated status)
